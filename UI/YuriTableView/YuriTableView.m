...
@interface YuriTableView ()

@end

@implementation YuriTableView
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        self.emptyView = [[YuriEmptyView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
        self.emptyView.backgroundColor = [UIColor whiteColor];
        [self.emptyView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(touchEmptyView:)]];
...
{
    [self openOrCloseCellWithTouchType:YuriCodeSendTouch forIndexPath:indexPath];
}
 ...
 */
- (void)openOrCloseCellWithTouchType:(YuriLineTouchType)touchType forIndexPath:(NSIndexPath *)indexPath
{
    switch (touchType)
    {
        case YuriHeaderLineTouch:
        {
            [self openOrCloseHeaderWithSection:indexPath.section];
        }
            break;
        case YuriCellLineTouch:
        {
            [self openOrCloseCellWithRow:indexPath.row];
        }
            break;
        case YuriCodeSendTouch:
        {
...
- (void)tableViewHeaderTouchUpInside:(UITapGestureRecognizer *)gesture
{
    ...
    [self openOrCloseCellWithTouchType:YuriHeaderLineTouch forIndexPath:indexPath];
}
...
- (void)tableViewCellTouchUpInside:(UITapGestureRecognizer *)gesture
{
    ...
    if(CGRectContainsPoint(rect,point))
    {
        if (self.atomView)
        {
            [self openOrCloseCellWithTouchType:YuriCellLineTouch forIndexPath:indexPath];
        }
...
- (void)invoke_emptyView {
    
    if ([self.dataSource respondsToSelector:@selector(mTableViewEmptyViewPlaceHodelImage:)]) {
        self.emptyView.emptyImageView.image = [self.dataSource mTableViewEmptyViewPlaceHodelImage:self];
    }
    if ([self.dataSource respondsToSelector:@selector(mTableViewEmptyViewTitleString:)]) {
        [self.emptyView.titleLabel setAttributedText:[self.dataSource mTableViewEmptyViewTitleString:self]];
    }
    if ([self.dataSource respondsToSelector:@selector(mTableViewEmptyViewInfoString:)]) {
        [self.emptyView.infoLabel setAttributedText:[self.dataSource mTableViewEmptyViewInfoString:self]];
    }
    if ([self.delegate respondsToSelector:@selector(mTableViewEmptyBackgroundColor:)]) {
        [self.emptyView setBackgroundColor:[self.delegate mTableViewEmptyBackgroundColor:self]];
    }
    
    if ([self.delegate respondsToSelector:@selector(mTableViewAllowTouchuEmptyView:)]) {
        
        self.allowTouchEmptyView = [self.delegate mTableViewAllowTouchuEmptyView:self];
    }
    
}

- (void)touchEmptyView:(UITapGestureRecognizer *)tap {
    
    if (self.allowTouchEmptyView && [self.delegate respondsToSelector:@selector(mTableViewDidTouchEmptyView:emptyView:)]) {
        [self.delegate mTableViewDidTouchEmptyView:self emptyView:self.emptyView];
    }
