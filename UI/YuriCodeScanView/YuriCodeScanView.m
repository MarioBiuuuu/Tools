...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        codeImageView.image = [UIImage imageNamed:@"scanBox.png"];
        ...
        CGFloat lineX = codeX + 20;
        CGFloat lineY = codeY;
        CGFloat lineW = codeW - 40;
        CGFloat lineH = 3;
...
//        }];
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"设备不可用" message:nil delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alert show];
...
- (void)setTipMessage:(NSString *)tipMessage {
    ...
    NSDictionary *attrs = @{NSFontAttributeName : self.tipLabel.font};
    CGFloat labelH = [self.tipLabel.text boundingRectWithSize:CGSizeMake(self.tipLabel.frame.size.width, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:attrs context:nil].size.height;
    self.tipLabel.frame = CGRectMake(self.tipLabel.frame.origin.x, self.tipLabel.frame.origin.y, self.tipLabel.frame.size.width, labelH);
}
