...
+(CustomEmojiView*)shardInstance{
    ...
    dispatch_once(&emojiviewOnce, ^{
        cutomemojiview = [[CustomEmojiView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 216.0f)];
    });
...
-(instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, [UIScreen mainScreen].bounds.size.width, 216.0f);
        ...
        CGFloat faceW = ([UIScreen mainScreen].bounds.size.width - leftMargin * 2.0) / 7.0;
        ...
        for (int p=0; p<pageCount; p++)
        {
            ...
            for (int j=0; j<row; j++)
            {
                ...
                {
                    NSInteger column_x = leftMargin + faceW * i;
                    if (number > 133)
                    {
                        break;
                    }
                    ...
                    {
                        UIButton* btn = [[UIButton alloc] initWithFrame:CGRectMake(page_X+column_x, row_y, faceW, 30.0f)];
                        btn.tag = number;
                        btn.backgroundColor = [UIColor clearColor];
                        btn.titleLabel.font = [UIFont systemFontOfSize:20.0f];
                        [btn setTitle:[EmojiCommonTools getExpressionStrById:number] forState:UIControlStateNormal];
            ...
            UIButton* delBtn = [[UIButton alloc] initWithFrame:CGRectMake(page_X+leftMargin + faceW * 6, 15+40*3, faceW, 30.0f)];
...
//        UIPageControl *pageView = [[UIPageControl alloc] initWithFrame:CGRectMake(100.0f, self.frame.size.height-40.0f, 120.0f, 20.0f)];
        CGFloat pageControlW = 120.f;
        CGFloat pageControlH = 20.f;
        CGFloat pageControlY = self.frame.size.height - 40.f;
        CGFloat pageControlX = ([UIScreen mainScreen].bounds.size.width - pageControlW) * 0.5;
        UIPageControl *pageView = [[UIPageControl alloc] initWithFrame:CGRectMake(pageControlX, pageControlY, pageControlW, pageControlH)];
        ...
        sendBtn.frame = CGRectMake(self.frame.size.width-80.0f, self.frame.size.height - 10 - 30, 60.0f, 30.0f);
